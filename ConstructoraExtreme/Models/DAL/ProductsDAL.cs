...
namespace ConstructoraExtreme.Models.DAL
{
    public class ProductsDAL
    {
        ...
        // Método para obtener un producto por su ID e incluir la información de la categoría.
        public async Task<Products> GetById(int id)
        {
            var product = await _context.Products
                .Include(p => p.Category) // Incluir la relación con la categoría
        ...
        public async Task<int> Edit(Products product)
        {
            ...
            if (productUpdate.Id != 0)
            {
                ...
                productUpdate.Category = product.Category; // Actualizar la relación con la categoría
        ...
        // Método privado para construir una consulta IQueryable con filtros, incluyendo la categoría.
        private IQueryable<Products> Query(Products product)
        {
            ...
            if (product.Category_Id > 0)
                query = query.Where(p => p.Category_Id == product.Category_Id); // Filtro por categoría
